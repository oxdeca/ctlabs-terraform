...
resource "google_dns_record_set" "ptr" {
  ...
  managed_zone = join("-", concat(["reverse"], reverse(split(".", google_compute_instance.vm[each.key].network_interface.0.network_ip))[0:ceil( tonumber(split(",", google_compute_subnetwork.subnet.ip_cidr_range)[1]) / 8 )]))
