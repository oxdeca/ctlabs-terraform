  ...
  metadata = merge(
     {
       ...
       ctlabs_base_disks = try( jsonencode([ for dk,dv in each.value.disks: { name = try(dv.name, "${each.value.name}-${dk}"), fstype = try(dv.fstype, local.defaults.disk.fstype), opts = try(dv.opts, local.defaults.disk.opts), path = try(dv.path, null), dv = dv } if !startswith(dk, "boot") ]), null )
