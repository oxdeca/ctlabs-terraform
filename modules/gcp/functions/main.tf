...
resource "google_project_service" "function" {
  for_each = toset(["cloudfunctions.googleapis.com", "cloudbuild.googleapis.com"])
...
resource "google_cloudfunctions_function" "func" {
  ...
  depends_on            = [google_project_service.function, google_storage_bucket_object.code, google_service_account.sa]
}
